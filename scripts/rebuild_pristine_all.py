...
def advanced_latexify(text):
    # Fix broken subscripts and vertical layouts
    ...
    # Clean floating isolated schematic labels & single-glyph line noise
    lines = text.splitlines()
    cleaned_lines = []
    for line in lines:
        l_s = line.strip()
        if l_s in [
            'I', 'v1', 'v2', 'v3', '2 Vx', 'Vx', 'vo(t)', '$v_o(t)$', 'vo', '$v_o$',
            '+', '-', '＋', '－', 'R1', 'R2', 'R3', 'RL', 'Zin', 'Is', 'V1', 'V2', 'V3', 'V4', 'V5',
            '[', ']', '{', '}', '|', 'T', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'ω', '＝', '='
        ]:
            continue
...
for folder, meta in subj_info.items():
    ...
    def generate_content(is_top_level=False):
        ...
        # Navigation table (Using Obsidian-safe anchors)
        lines.append('## 📑 快速目錄導覽\n')
        ...
        for data in years_data:
            ...
            anchor_text = f'{y} 年 電機工程技師 — {meta["name"]}'
            lines.append(f'| **{y} 年** | {meta["name"]} | `{data["exam_code"]}` | 2 小時 | {data["calc_rule"]} | [[#{anchor_text}|🔗 前往 {y} 年試題]] | [📄 {y}年 PDF]({pdf_link}) |')
        ...
        for data in years_data:
            ...
            if p_count == 1:
                img1 = f'{bname}_p1.png'
                lines.append(f'![[{img1}|750]]\n')
            elif p_count == 2:
                img1 = f'{bname}_p1.png'
                img2 = f'{bname}_p2.png'
                lines.append(f'![[{img1}|750]]\n\n![[{img2}|750]]\n')
            elif p_count == 3:
                img1 = f'{bname}_p1.png'
                img2 = f'{bname}_p2.png'
                img3 = f'{bname}_p3.png'
                lines.append(f'![[{img1}|750]]\n\n![[{img2}|750]]\n\n![[{img3}|750]]\n')
            
            lines.append('[[#📑 快速目錄導覽|⬆ 回到目錄導覽]]\n')
            lines.append('---\n')
...
print('All 6 subjects rebuilt with clean LaTeX, Obsidian anchors, and foolproof image embeds!')
