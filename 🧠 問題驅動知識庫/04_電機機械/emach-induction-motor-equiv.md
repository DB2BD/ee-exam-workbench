---
generated: true
generator: scripts/generate_obsidian_knowledge.py
nodeId: emach-induction-motor-equiv
nodeType: mechanism
examFamily: PE
lifecycle: active
graphRevision: kg-v1-6f7a29de450d7597
sourceHash: ba4ce757c4707fb8e043627cf0d3c745df7ca3014dfa5f8e1d377b6603ff5b2f
generatedBodyHash: 140706cbcd2fd70fd9e966266dd42e2d7b2115213cd9622c19bd2fffa58e714c
---
# 感應電動機等效電路與戴維寧化簡

> 此檔案由 canonical knowledge graph 產生；請將個人補充寫在 `📝 個人知識補充/`。

## Canonical identity
- node: `emach-induction-motor-equiv`
- type: `mechanism`
- exam family: `PE`
- lifecycle: `active`

## Semantic links
- `prerequisite` → [[emach-induction-motor-torque]] — 掌握「感應電動機轉矩-轉差率曲線與最大轉矩」前，先建立「感應電動機等效電路與戴維寧化簡」這個前置概念。

## Linked questions
- [[EE-104-04-4]]（confidence: 0.92）
- [[EE-106-04-4]]（confidence: 0.92）
- [[EE-107-04-3]]（confidence: 0.92）
- [[EE-108-04-3]]（confidence: 0.9）
- [[EE-109-04-3]]（confidence: 0.92）
- [[EE-109-04-4]]（confidence: 0.92）
- [[EE-112-04-4]]（confidence: 0.92）
- [[EE-113-04-4]]（confidence: 0.96）
- [[EE-113-04-5]]（confidence: 0.9）
- [[EE-114-04-3]]（confidence: 0.92）

## Provenance
- legacyNodeId: emach-induction-motor-equiv
- sourcePath: src/data/knowledge-dag.js
- sourceType: legacy-dag
