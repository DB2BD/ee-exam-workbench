...
def advanced_latexify(text):
    # Fix broken subscripts and vertical layouts from PDF text extraction
    ...
    # Common Electrical & Physical Units in LaTeX format
    ...
    # Time-domain voltage and current signals
    ...
    # Clean floating isolated schematic labels
    lines = text.splitlines()
    cleaned_lines = []
    for line in lines:
        l_s = line.strip()
        if l_s in ['I', 'v1', 'v2', 'v3', '2 Vx', 'Vx', 'vo(t)', '$v_o(t)$', 'vo', '$v_o$', '+', '-', '＋', '－', 'R1', 'R2', 'R3', 'RL', 'Zin', 'Is', 'V1', 'V2', 'V3', 'V4', 'V5']:
            continue
...
def parse_exam_file(pdf_path, subject_folder, year_str):
    ...
    # Major question formatting
    body = re.sub(r'(?:^|\n)\s*([一二三四五六七八九十]+[、\.])\s*', r'\n\n#### \1 ', body)
    
    # LaTeX formatting
    body = advanced_latexify(body)
    
    # Specific subject polish
    if subject_folder == '03_工程數學':
        if year_str == '114':
            ...
            body = re.sub(r'二、\s*試求一時間函數[\s\S]*?之拉普拉斯轉換[^\n]*',
    ...
    # Ensure headings don't have duplicate ####
...
for folder, meta in subj_info.items():
    ...
    def generate_content(is_top_level=False):
        ...
        for data in years_data:
            ...
            anchor = f'#{y}-年-電機工程技師--{meta["name"]}'
            lines.append(f'| **{y} 年** | {meta["name"]} | `{data["exam_code"]}` | 2 小時 | {data["calc_rule"]} | [🔗 前往 {y} 年試題]({anchor}) | [📄 {y}年 PDF]({pdf_link}) |')
        ...
        for data in years_data:
            ...
            lines.append('### 📷 官方試卷與電路圖檔對照（點擊可查看原始高解析度大圖）\n')
            
            p_count = data['page_count']
            if p_count == 1:
                img1 = f'{img_prefix}{bname}_p1.png'
                lines.append(f'| 試卷第 1 頁（含完整電路與圖表） |\n|:---:|\n| [![第1頁]({img1})]({img1}) |\n')
            elif p_count == 2:
                img1 = f'{img_prefix}{bname}_p1.png'
                img2 = f'{img_prefix}{bname}_p2.png'
                lines.append(f'| 試卷第 1 頁 | 試卷第 2 頁 |\n|:---:|:---:|\n| [![第1頁]({img1})]({img1}) | [![第2頁]({img2})]({img2}) |\n')
            elif p_count == 3:
                img1 = f'{img_prefix}{bname}_p1.png'
                img2 = f'{img_prefix}{bname}_p2.png'
                img3 = f'{img_prefix}{bname}_p3.png'
                lines.append(f'| 試卷第 1 頁 | 試卷第 2 頁 | 試卷第 3 頁 |\n|:---:|:---:|:---:|\n| [![第1頁]({img1})]({img1}) | [![第2頁]({img2})]({img2}) | [![第3頁]({img3})]({img3}) |\n')
...
print('All Markdown files completely updated and pristine!')
