...
> [!NOTE]
> 第 2 題（`EE-106-02-2`）已依參考書更新為對稱 MOSFET 非反相負回授的符號式主解：\(A=\frac12g_{mN}r_o\)、\(\beta=R_1/(R_1+R_2)\)、\(A_f=A/(1+A\beta)\)。完整輸出阻抗式請見[題號級 canonical 詳解](canonical/EE-106-02-2.md)。
...
> [!NOTE] 本題依參考書以符號式回答；官方裁切圖未提供數值參數，請以 [EE-106-02-2 canonical](canonical/EE-106-02-2.md) 的對稱小訊號模型為準。
...
回授因數： $\beta=R_1/(R_1+R_2)$。令 $A_{ol}$ 為含 $M_1\sim M_4$、$r_o$、尾電流源及分壓器負載的完整開路增益，則
$$
\mathbf{A_f=\frac{A_{ol}}{1+A_{ol}\beta}}.
$$
只有 $|A_{ol}\beta|\gg1$ 時才有 $A_f\approx1/\beta=1+R_2/R_1$。
...
$$
其中 $R_{out,ol}$ 必須包含 $r_{o2},r_{o4}$、電流鏡及 $R_1+R_2$ 的完整小訊號等效。
...
> [!NOTE]
> 參考書主解採 $A=\frac12g_{mN}r_o$、$\beta=R_1/(R_1+R_2)$ 的對稱模型。官方裁切圖沒有數值參數，所以答案維持符號式，不補造數值。
