...
3. `QuestionRecord` 將識別、內容、taxonomy、solution、provenance 與 verification 語意分層；正式稽核狀態使用 `verified`、`reference_book_verified`、`needs_manual_review`、`suspected_error`、`not_attempted`。舊資料的 `in_progress`、`pending`、`ambiguous`、`unavailable` 可暫存，但不能被誤報為已驗證。
...
7. 複習提取狀態獨立儲存於 `recallState`：每一級都必須重新累積連續兩次達標才升級，升級後 streak 歸零，失敗只退一級。`revealStep` 記錄本次介面實際揭露進度；`lastAchieved` 則由 1／3／5 自評映射為 0／2／4，表示揭露前的回想成果，並同時作為間隔排程輸入，兩者不得互相冒充。舊版無 `streakVersion` 的紀錄採保守遷移：保留層級與歷史次數、將 streak 歸零。分類器使用 alias 正規化、0.65 信心與 0.15 margin 門檻，低信心題進入人工複核。
