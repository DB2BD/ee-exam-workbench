...
review_disposition: curve_interpolation_branches
review_blocker: graph_estimate
review_action: 確認激磁電阻設定與磁化曲線取點／插值規則；目前保留曲線估讀區間。
review_evidence: 官方裁切圖包含 1800 rpm 磁化表的離散 (I_f,E_a) 點；canonical 已重算 R_a+R_sr 壓降、無載最大電壓及長分路場電流，並同時列出最近表格點 1.25 A 與線性插值 1.254 A。缺口是題目未指定曲線取點／插值與 R_fc 設定。
---
