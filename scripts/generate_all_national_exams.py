# -*- coding: utf-8 -*-
"""
generate_all_national_exams.py
==============================
Generates all 25 official Gaokao Level 3 (公務高考三級電力工程) exam Markdown files
covering 110 ~ 114 (5 years × 5 subjects) with full, standardized KaTeX formulas.
"""
...
for d in SUBJECT_DIRS.values():
    os.makedirs(os.path.join(BASE_DIR, d), exist_ok=True)

# Define full exam data
EXAM_DATA = {
    # ══════════════════════════════════════════════════════════════════
    # 01. 電路學 (110 ~ 114)
    # ══════════════════════════════════════════════════════════════════
...
def generate_markdown(sid, yr, data):
    subj_folder = SUBJECT_DIRS[sid]
    filename = f"GK_{yr}年_{data['title']}.md"
    filepath = os.path.join(BASE_DIR, subj_folder, filename)

    lines = [
        f"# 🏛️ 公務人員高等考試三級（電力工程）歷屆試題 — {data['title']}（{yr}年）",
        "",
        "> **等別**：高等考試三級  ",
        f"> **類科**：電力工程 / 電子工程  ",
        f"> **科目**：{data['title']}  ",
    ...
    lines.extend([
        "### 📷 官方試卷圖檔",
        "",
        f"![[GK_{yr}年_{data['title']}_p1.png|750]]",
        ""
    ])
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"  ✅ Created {subj_folder}/{filename} ({len(data['questions'])} questions)")

print("🚀 Generating 25 National Exam Markdown Files (110~114 年)...")
for (sid, yr), data in sorted(EXAM_DATA.items()):
    generate_markdown(sid, yr, data)

print(f"\n🎉 All {len(EXAM_DATA)} exam Markdown files successfully written to 依考科分類/🏛️_國考同級參考題庫/")
