...
3. **Collect artifact files**
   ...
   Use cached scope `path` and `old_path` entries under `docs/spectra/changes/<name>/` as artifact files. If Git status is needed, parse `git status --porcelain=v1 -z` as NUL records with both rename paths, treating whitespace/newlines as filename data. Cache the proposal summary and task progress for both normal and archive commit messages.
