...
class TestKnowledgeGraphGeneration(unittest.TestCase):
    ...
    def test_valid_graph_produces_byte_stable_bundle_and_report(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("kg-v1-97a9a9a3fcd6334b", first.read_text(encoding="utf-8"))
            ...
            self.assertEqual(result["graphRevision"], "kg-v1-97a9a9a3fcd6334b")
            self.assertEqual(result["mappedQuestionCount"], 482)
            self.assertEqual(result["unknownQuestionCount"], 0)
            self.assertEqual(result["coverage"]["actual"], 482)
            self.assertEqual(result["coverage"]["required"], 482)
