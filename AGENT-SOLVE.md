# 電機工程技師考古題標準題解 SOP 與解題規範 (AGENT-SOLVE.md)

本文件定義本知識庫所有考古題解析的**標準解題 SOP（Standard Operating Procedure）**、格式規範與驗證維度。它用來提高推導的可追溯性與一致性，不宣稱任何題解等同官方標準答案或保證考試得分。

## 零、先確認來源與驗證邊界

開始推導前，先定位官方 PDF／原題裁切圖、題目頁碼、參考書頁碼與目前狀態。參考書可作為核對材料，但不能取代原題來源；若圖片模糊、參數缺漏、參考書與獨立驗算衝突，必須保留假設、條件式分支並標記 `needs_manual_review` 或 `suspected_error`。完成參考書核對但尚未具備足夠官方證據時，使用 `reference_book_verified`，不要直接寫成 `verified`。
