...
subjects = [
    ...
    ('04', '電機機械', '⚙️', '#a17846', '依考科分類/04_電機機械.md', '依考科分類/04_電機機械'),
...
def estimate_difficulty(topic, q_body):
    diff = 3
    hard_words = ['SVD', '非齊次', '凸極', '牛頓法', '快速解耦', 'SPWM', '反時限', '零空間', '留數', '特徵值對角化', '雙反應', '自耦變壓器', '搖擺方程', '暫態穩定', '保護協調']
    easy_words = ['戴維寧等效', '節點電壓', '一階ODE', '分壓', '單相二線', '歐姆定律', 'KVL', 'KCL', '變壓器變比']
        ...
        for i in range(1, len(year_sections), 2):
            ...
            if len(q_blocks) > 1:
                for j in range(1, len(q_blocks), 2):
                    ...
                    difficulty = estimate_difficulty(topic, q_body)
...
sevenLayers = [
    ...
    { "id": "L7", "title": "💼 職涯與資產總藍圖", "desc": "中鼎工程躍升 ➔ 2027 技師優利貸款 ➔ 30 歲突破 305 萬資產終極藍圖", "link": "💼 個人職涯發展與國際戰略/🌟_電機工程師職涯躍升與資產質押複利_終極整合總藍圖.md" }
]
