...
1. **含非額定變比變壓器 $\pi$ 型等效電路模型**：
   ...
   $$\mathbf{Y}_{branch} =$$
$$
\begin{bmatrix} \\ \frac{y}{a^2} & -\frac{y}{a} \\ -\frac{y}{a} & y \end{bmatrix}
$$=$$
...
$$(\delta_{cr} - \delta_0) + \cos\delta_{cr} - \cos\delta_0 = 1.5625\cos\delta_{cr} - 1.5625\cos\delta_0_{max} - \delta_{max} + \delta_{cr}$$
兩邊消去 $\delta_{cr}$：
