...
> **104～114 年｜6 大考科｜官方原卷可追溯｜完整資料夾可離線使用**
...
| 你想做的事 | 直接入口 |
| :--- | :--- |
| 直接照 115 年考期執行，不自行排程 | [52 天、69 小時免回填倒數節奏](./docs/上榜倒數節奏_115年電機技師.md) |
| 不等回填，直接開始固定上榜路徑 | [24 個核心時段＋混合橋接＋6 科模考](./docs/上榜預設24時段_核心題路徑.md) |
| 每題先留下可閱卷的得分證據 | [六科考場得分骨架](./docs/上榜考場得分骨架_六科.md) |
| 確認核心章節仍在現行官方範圍 | [12 核心章節與命題大綱對照](./docs/上榜核心路徑_現行命題大綱對照.md) |
| 核心題後先練無章節提示辨識 | [六科 12 題混合橋接](./docs/上榜混合橋接_六科12題.md) |
| 核心時段後做最新年度計時演練 | [114 年六科被動模考執行包](./docs/上榜被動模考_114年六科執行包.md) |
| 用零重疊試卷檢查方法能否移轉 | [108 年六科被動複測執行包](./docs/上榜被動複測_108年六科執行包.md) |
| 模考後直接修復錯因 | [114／108 年 59 題錯因修復索引](./docs/上榜錯因修復索引_114-108.md) |
| 遇到題幹缺條件時安全拿分 | [7 題條件題安全作答卡](./docs/上榜精確解答邊界_條件題處理.md) |
| 自選記錄六科程度（非前置條件） | [114 年六科限時診斷與記錄表](./docs/上榜基線_114年六科診斷.md) |
| 查看核心題的歷屆覆蓋依據 | [104～114 年核心母題候選](./docs/上榜核心母題候選_104-114年.md) |
...
| 查看已發布版本變更 | [v1.0.7 發版紀錄](./docs/發版紀錄_v1.0.7.md) |

## 目前版本與資料範圍

目前公開發版紀錄：**v1.0.7**（2026-09-12）；工作台實際建置識別以頁面與部署 commit 為準。

| 資料集 | 規模 | 用途 |
| :--- | :---: | :--- |
| **PE／EE 技師題庫** | 104～114 年、6 科、323 題 | 技師歷屆申論題、原卷對照與逐題詳解 |
| **PE 題解稽核** | 323 題有題級稽核記錄 | 298 題 `verified`、15 題 `reference_book_verified`、10 題保留人工覆核（截至 2026-09-25） |
...
- **完整資料夾可離線使用**：下載或 clone 整個專案後開啟 [`index.html`](./index.html)。工作台會相對引用四個資料 bundle、`libs/`、原題裁切圖與本地 PDF；只下載 `index.html` 並不是完整離線包。實際載入時間依裝置、瀏覽器與快取狀態而異。
...
複習頁的「開始提取訓練」會依序提供四層揭示：**章節 → 起手式 → 核心公式／陷阱 → 完整推導**。每題完成後以 1／3／5 回顧揭露前的作答狀況，並可記錄「題型辨識錯、起手式不會、公式忘記、計算錯、觀念混淆」之一；提取掌握度 L1～L4 在每一級都須重新連續兩次達標才升級，失敗只退一級。每日練習預設只進入 7 天避重，自評「無法完成」後可自行選擇加入到期複習；到期複習的三種評分才會直接更新間隔日期。
...
CI 通過完整測試與資產閘門後，會以 `scripts/stage_pages_artifact.py` 建立 `_site/` 再發布，不再上傳整個 Git 工作目錄。staging 明確保留 `index.html`、四個資料／題解 bundle、`libs/`、官方 PDF、題目裁切與題解引用圖片；缺少任何執行期映射資產時會直接失敗。效能與體積紀錄見 [`docs/PERFORMANCE_BASELINE_2026-09-08.md`](docs/PERFORMANCE_BASELINE_2026-09-08.md)。
...
這套順序不等待自評、不要求把分數或錯因回填到專案，也不先猜個人強弱：

0. 115 年應考者直接照 [52 天逐日開工表](./docs/上榜逐日開工表_115年.md) 執行；每天只有一個固定任務或恢復日，不必在 [69 小時倒數節奏](./docs/上榜倒數節奏_115年電機技師.md) 的日期區段內自行排課。
1. 依序完成 [24 個核心時段](./docs/上榜預設24時段_核心題路徑.md)，建立 12 個高年度覆蓋章節的母題、同型題與變式題骨架。
2. 完成 [六科 12 題混合橋接](./docs/上榜混合橋接_六科12題.md)，拿掉章節提示後重做變式題，確認能否自己選對起手式。
3. 完成 [114 年六科計時模考](./docs/上榜被動模考_114年六科執行包.md)，每科 120 分鐘統一採 [5＋90＋17＋8 得分節奏](./docs/上榜考場120分鐘得分節奏.md)，再做 30 分鐘核對與 30 分鐘重寫；其中 5 題與核心路徑重疊，所以定位為演練而非陌生題基線。
4. 完成 [108 年六科被動複測](./docs/上榜被動複測_108年六科執行包.md)；30 題與核心路徑零重疊，用來檢查方法能否移轉，不用跨年度原始總分差宣稱進步。
5. 所有失分題直接查 [59 題錯因修復索引](./docs/上榜錯因修復索引_114-108.md)；其中 15 題若卡在起手式或時間不足，先看 [起手式急救卡](./docs/上榜起手式急救卡_114-108.md)，只取得第一個可得分動作，再閉卷續寫。只重寫最高配分缺口，然後照固定順序前進。
6. 若日後在全題庫遇到題幹本身缺參數的 7 題，直接用 [條件題安全作答卡](./docs/上榜精確解答邊界_條件題處理.md)，先寫假設再寫條件解，不影響前五步順序。

歷屆覆蓋只決定練習優先序，不是未來命中率；公式、列式、單位、方向與結論以題號級 `verified` 題解為準。個人進度儀表板、收藏與 AI 批改是可選工具，不是開始或完成上述路徑的前置條件。
