...
def extract_clean_exam_content(pdf_path):
    ...
    for p_idx, page in enumerate(doc):
        ...
        for line in lines:
            ...
            if any(kw in l for kw in [
                ...
                continue
            # Header prefixes and single tokens
            ...
            if re.match(r'^\(\w+\)(?:可以使用|禁止使用|不必抄題|本科目除)', l):
                continue
            clean_page_lines.append(l)
        
        page_texts.append('\n'.join(clean_page_lines))
    
    body = '\n\n'.join(page_texts).strip()
    
    # Format sub-items (一) ~ (十)
    body = re.sub(r'(?:^|\n)\s*(\([一二三四五六七八九十]+\))\s*', r'\n\n* **\1** ', body)
    
    # Format major questions 一、 ~ 十、
    body = re.sub(r'(?:^|\n)\s*([一二三四五六七八九十]+[、\.])\s*', r'\n\n#### \1 ', body)
...
for folder, meta in subj_info.items():
    ...
    for pdf in pdfs:
        ...
        parsed = extract_clean_exam_content(pdf)
    ...
    def make_markdown(is_top_level=False):
        ...
        for data in years_data:
            ...
            lines.append('### 📷 官方試卷與電路圖檔對照（點擊可查看原始解析度大圖）\n')
            
            p_count = data['page_count']
            if p_count == 1:
                img1 = f'{img_prefix}{bname}_p1.png'
                lines.append(f'| 試卷第 1 頁（含電路與圖表） |\n|:---:|\n| [![第1頁]({img1})]({img1}) |\n')
            elif p_count == 2:
                img1 = f'{img_prefix}{bname}_p1.png'
    ...
    # Write subject folder markdown
    subj_file_path = os.path.join(subj_path, f'{folder}_歷屆試題彙編_104-114年.md')
    with open(subj_file_path, 'w', encoding='utf-8') as f:
...
print('All 6 subject markdown files built successfully!')
