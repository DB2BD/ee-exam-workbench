---
generated: true
generator: scripts/generate_obsidian_knowledge.py
nodeId: dist-short-circuit-capacity
nodeType: procedure
examFamily: PE
lifecycle: active
graphRevision: kg-v1-6f7a29de450d7597
sourceHash: 01e29f311532bf5eb0705996e30757a734a79c7e6d208bc02b49e6686d732280
generatedBodyHash: 8751a48ac09b8612d317f2c9e43d58279c37f83d9f6f9dc921d663ab9e97e2b7
---
# 短路容量計算 (MVA 法)

> 此檔案由 canonical knowledge graph 產生；請將個人補充寫在 `📝 個人知識補充/`。

## Canonical identity
- node: `dist-short-circuit-capacity`
- type: `procedure`
- exam family: `PE`
- lifecycle: `active`

## Semantic links
- `prerequisite` → [[dist-arc-flash-ieee80]] — 掌握「IEEE Std 80 變電所接地網跨步/接觸安全電壓」前，先建立「短路容量計算 (MVA 法)」這個前置概念。
- `prerequisite` → [[dist-protection-coordination]] — 掌握「過電流電驛與保護協調 (TCC 曲線)」前，先建立「短路容量計算 (MVA 法)」這個前置概念。

## Linked questions
- [[EE-104-06-4]]（confidence: 0.9）
- [[EE-105-06-3]]（confidence: 0.9）
- [[EE-106-06-1]]（confidence: 0.92）
- [[EE-106-06-2]]（confidence: 0.96）
- [[EE-108-06-4]]（confidence: 0.9）
- [[EE-109-06-3]]（confidence: 0.9）
- [[EE-110-06-5]]（confidence: 0.96）
- [[EE-111-06-3]]（confidence: 0.96）
- [[EE-112-06-3]]（confidence: 0.9）
- [[EE-112-06-4]]（confidence: 0.92）
- [[EE-113-06-4]]（confidence: 0.9）
- [[EE-114-06-3]]（confidence: 0.92）

## Provenance
- legacyNodeId: dist-short-circuit-capacity
- sourcePath: src/data/knowledge-dag.js
- sourceType: legacy-dag
