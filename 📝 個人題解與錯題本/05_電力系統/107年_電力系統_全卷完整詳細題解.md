...
### ✏️ 步驟式詳細數學推導（canonical 校驗版）

> 本節以發電機端 24 kV 為基準；變壓器低壓額定為 25 kV，因此 345 kV 無限匯流排折算為 $25/24$ pu。原年度模板把基準差異與無效功率方向混用，以下結果以題圖及相量方程重新回代。

1. **變壓器基準修正與功角**：
   $$X_T'=0.20\left(\frac{25}{24}\right)^2=0.2170139\text{ pu},\qquad P=\frac{800}{850}=0.941176\text{ pu}.$$
   $$\sin\delta=\frac{P X_T'}{|V_t||V_\infty|'}=\frac{0.941176(0.2170139)}{1.041667}=0.196078,$$
   $$\delta=11.3077^\circ.$$
2. **發電機端無效功率**（正號為送出、負號為吸收）：
   $$Q_G=\frac{|V_t|^2-|V_t||V_\infty|'\cos\delta}{X_T'}=-0.0988234\text{ pu},$$
   $$\boxed{Q_G=-84.0\text{ Mvar}}.$$
   負號表示發電機端吸收無效功率，即進相（leading）運轉。交叉檢查 $|I|=\sqrt{P^2+Q_G^2}=0.94635$ pu，並以 $X_d=1.0$ pu 回算 $|E'|=1.30305$ pu。
3. **變壓器容量**：
   $$S_T=\sqrt{800^2+84.0^2}=\boxed{804.40\text{ MVA}}<850\text{ MVA},$$
   故負載率約 $94.64\%$，未過載。

---

### 🎯 第三題滿分關鍵與結論
- **發電機無效功率**：$Q_G=\mathbf{-84.0\text{ Mvar}}$（進相 Leading）
- **變壓器視在功率**：$S_T=\mathbf{804.40\text{ MVA}}$（安全未過載）
- 舊版 $+82.6$ Mvar 僅保留為歷史錯誤示例，不得作答。
