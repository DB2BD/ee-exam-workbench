...
系統最初僅收錄「電機工程技師（104~114 年共 321 題）」。目前 GK 已形成 161 筆記錄（101 道申論題與 60 道工程數學測驗題）。若直接把它們合併進原有的 `dashboard-data.js` 與 `solutions-bundle.js`，將帶來高度風險：
1. **污染原始核心資料庫**：可能導致 321 題技師資料（含已驗證與人工複核狀態）損壞或格式不相容。
