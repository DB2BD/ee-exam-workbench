...
review_evidence: 官方圖的線路電抗與 Bus 2 PV、Bus 3 PQ 條件已重建成 Y_bus，Newton 法得到高／低電壓兩個正值根；兩根殘差均達數值容許，且 Jacobian 最小奇異值分別 0.59230 與 0.05141。考選部原始檔明載本題為承上題並指定 Bus 1 為無限匯流排、Bus 2 為發電機匯流排；題目仍未指定運轉分支。來源：https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?c=011&code=106180&q=1&s=0611&t=Q
---
...
- 「承上題」與正常電力系統運轉語境使高電壓根成為合理主答案候選；但題面仍未指定 Newton 初值或明示穩定運轉分支，狀態維持 `needs_manual_review`，避免把未指定分支誤稱唯一答案。
